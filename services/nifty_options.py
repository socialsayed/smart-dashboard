# =====================================================
# NSE NIFTY OPTIONS (HARDENED – P3-4)
# =====================================================
...
def _get_nse_session():
    ...
    if _nse_session is None:
        ...
        try:
            ...
            logger.warning("NSE cookie priming failed")
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                logger.warning(
                    f"NSE option chain HTTP {r.status_code} (attempt {attempt+1})"
                )
            ...
            if not spot or not expiry_dates or not rows_raw:
                last_error = "Incomplete NSE response"
                logger.warning(
                    f"NSE option chain incomplete data (attempt {attempt+1})"
                )
                time.sleep(1 + attempt)
                continue
            ...
            logger.warning(f"NSE option chain empty rows (attempt {attempt+1})")

        except requests.Timeout:
            last_error = "Timeout"
            logger.warning(
                f"NSE option chain timeout (attempt {attempt+1})"
            )

        except Exception:
            last_error = "Unexpected failure"
            logger.exception(
                f"NSE option chain unexpected failure (attempt {attempt+1})"
            )

        time.sleep(1 + attempt)

    logger.error(f"NSE option chain failed after retries: {last_error}")
